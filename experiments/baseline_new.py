...
class BiCrossAttnPooling(nn.Module):
    ...
    def forward(self, text, audio, vision, audio_lengths, vision_lengths):
        ...
        feats = [z_a_att, z_t_att, z_a_pool]
...
class MandarinHybridFusionV3(nn.Module):
    """
    Kernel + BiCross + LowRank (3-way expert mixture)

    f_mix = a1*f_kernel + a2*f_bi + a3*f_lr
    """
    ...
    def __init__(
        ...
        # -------------------
        self.gate = nn.Sequential(
            nn.Linear(9 * d_model, hidden_dim),
            nn.ReLU(),
            nn.Dropout(dropout_head),
        ...
        # 5) Shared head
        # -------------------
        self.head = SharedPredictionHead(
            in_dim=3 * d_model,
            hidden_dim=hidden_dim,
    ...
    def forward(self, text, audio, vision, audio_lengths, vision_lengths):
        ...
        # ----- branch outputs -----
        f_kernel = self.kernel.forward_features(text, audio, vision, audio_lengths, vision_lengths)

        f_bi = self.bicross(text, audio, vision, audio_lengths, vision_lengths)
        f_bi = self.proj_bicross_to_3d(f_bi)

        f_lr = self.tri.forward_features(text, audio, vision, audio_lengths, vision_lengths)

        # ----- 3-way gating -----
        gate_input = torch.cat([f_kernel, f_bi, f_lr], dim=-1)  # (B, 9D)
        alpha = torch.softmax(self.gate(gate_input), dim=-1)   # (B, 3)

        a1 = alpha[:, 0:1]
        a2 = alpha[:, 1:2]
        a3 = alpha[:, 2:3]

        f_mix = a1 * f_kernel + a2 * f_bi + a3 * f_lr  # (B, 3D)

        # ----- prediction -----
        yhat = self.head(f_mix)
        return yhat

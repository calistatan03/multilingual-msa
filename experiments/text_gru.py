...
from encoders.audio import AudioEncoder
from encoders.vision import VisionEncoder
from encoders.text_gru import TextBiGRUEncoder
...
class BiCrossAttnPooling(nn.Module):
    ...
    def __init__(
        ...
        self.text_tok = TextBiGRUEncoder(in_dim=text_in, d_model=d_model, dropout=dropout_enc)
    ...
    def forward(self, text, audio, vision, audio_lengths, vision_lengths):
        ...
        z_t_pool = masked_mean_pool(H_t, valid_t) 
        # audio -> text
        ...
        feats = [z_t_att, z_a_att]

        # optional audio -> vision
        if self.include_audio_to_vision:
            print("Audio to vision attention included")

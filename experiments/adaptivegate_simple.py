...
class BiCrossAttnPooling(nn.Module):
    ...
    def __init__(
        ...
        self.n_blocks = 3 + (1 if include_audio_to_vision else 0) + (1 if include_pooled_text else 0)

        self.component_gate = nn.Sequential(
            nn.Linear(self.n_blocks * d_model, d_model),
            nn.ReLU(),
    ...
    def forward(self, text, audio, vision, audio_lengths, vision_lengths):
        ...
        # stack feature blocks: (B, n_blocks, D)
        feat_tensor = torch.stack(feats, dim=1)

        # flatten for gate input: (B, n_blocks * D)
        gate_in = feat_tensor.reshape(feat_tensor.size(0), -1)

        # gate values: (B, n_blocks)
        gate = torch.sigmoid(self.component_gate(gate_in))

        # apply one scalar gate per block
        gated_feats = feat_tensor * gate.unsqueeze(-1)                           # (B, n_blocks, D)

        # optional: save for analysis
        self.last_component_gate = gate.detach()

        return gated_feats.reshape(gated_feats.size(0), -1)

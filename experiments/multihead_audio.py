...
class CrossModalMultiHeadAttention(nn.Module):
    ...
    Multi-head cross-attention:
      q comes from one modality, kv comes from another modality.

    q:  (B, Tq, D)
    kv: (B, Tk, D)
    kv_valid: (B, Tk) bool, True for valid positions
    ...
    def forward(self, q: torch.Tensor, kv: torch.Tensor, kv_valid: torch.Tensor = None):
        B, Tq, D = q.shape
        _, Tk, _ = kv.shape

        Q = self.q_proj(q)   # (B,Tq,D)
        K = self.k_proj(kv)  # (B,Tk,D)
        V = self.v_proj(kv)  # (B,Tk,D)

        Q = Q.view(B, Tq, self.num_heads, self.head_dim).transpose(1, 2)  # (B,H,Tq,Hd)
        K = K.view(B, Tk, self.num_heads, self.head_dim).transpose(1, 2)  # (B,H,Tk,Hd)
        V = V.view(B, Tk, self.num_heads, self.head_dim).transpose(1, 2)  # (B,H,Tk,Hd)

        scores = torch.matmul(Q, K.transpose(-2, -1)) / math.sqrt(self.head_dim)  # (B,H,Tq,Tk)

        if kv_valid is not None:
            pad_mask = ~kv_valid[:, None, None, :]  # True where padding
        ...
        out = self.o_proj(out)
        out = self.norm(q + self.out_dropout(out))  # residual + norm
...
class BiCrossAttnPoolingAudioMHA(nn.Module):
    """
    Audio-guided multi-head cross-attention branch.

    Uses pooled audio as the anchor/query:
      - audio -> text
      - optional audio -> vision

    Returns concatenated fixed-length features.
    """
    def __init__(
        self,
        d_model=256,
        ...
        self.text_tok = TextTokenProjector(
            in_dim=text_in,
            d_model=d_model,
            dropout=dropout_enc,
        )
        self.text_pool = TextEncoder(
            in_dim=text_in,
            out_dim=d_model,
            dropout=dropout_enc
        ) if self.include_pooled_text else None

        self.attn_a2t = CrossModalMultiHeadAttention(
            d_model=d_model,
            num_heads=num_heads,
            dropout=dropout_enc,
        )
        self.attn_a2v = CrossModalMultiHeadAttention(
            d_model=d_model,
            num_heads=num_heads,
            dropout=dropout_enc,
        )
    ...
    def forward(self, text, audio, vision, audio_lengths, vision_lengths):
        # audio sequence and pooled audio anchor
        H_a, valid_a = self.audio_enc(audio, audio_lengths, return_seq=True)   # (B,Ta,D), (B,Ta)
        z_a_pool = masked_mean_pool(H_a, valid_a)                              # (B,D)
        q_a = z_a_pool.unsqueeze(1)                                            # (B,1,D)

        # text token sequence
        H_t, valid_t = self.text_tok(text)                                     # (B,Tt,D), (B,Tt)

        # audio -> text (pooled audio query attends over text tokens)
        H_at, attn_at = self.attn_a2t(q_a, H_t, kv_valid=valid_t)              # (B,1,D)
        z_t_att = H_at.squeeze(1)                                              # (B,D)

        feats = [z_t_att, z_a_pool]

        # optional audio -> vision
        if self.include_audio_to_vision:
            H_v, valid_v = self.vision_enc(vision, vision_lengths, return_seq=True)
            H_av, attn_av = self.attn_a2v(q_a, H_v, kv_valid=valid_v)          # (B,1,D)
            z_v_att = H_av.squeeze(1)                                          # (B,D)
            feats.append(z_v_att)
        else:
            attn_av = None

        # optional pooled text anchor
        if self.text_pool is not None:
            z_t_pool = self.text_pool(text)                                    # (B,D)
            feats.append(z_t_pool)

        # save attention maps for later analysis
        self.last_attn_at = attn_at.detach()
        self.last_attn_av = attn_av.detach() if attn_av is not None else None

        return torch.cat(feats, dim=-1)

class BiCrossAttnPooling(nn.Module):
    """
    Uses your GuidedTemporalAttention to compute:
      - audio -> text pooling (audio query attends over text tokens)
      - text  -> audio pooling (text query attends over audio frames)
    Optionally:
      - audio -> vision pooling

    Returns a concatenated feature vector:
      [z_t_att, z_a_att, z_a_pool, (z_v_att), (z_t_pool)]
    """
    def __init__(
        self,
        d_model=256,
        ...
        self.audio_enc = AudioEncoder(in_dim=audio_in, d_model=d_model, kernel_size=audio_kernel, dropout=dropout_enc)
        self.vision_enc = VisionEncoder(in_dim=vision_in, d_model=d_model, kernel_size=vision_kernel,
    ...
    def forward(self, text, audio, vision, audio_lengths, vision_lengths):
        ...
        # text -> audio (query: pooled text if available else masked-mean of token seq)
        z_t_query = z_t_pool if z_t_pool is not None else masked_mean_pool(H_t, valid_t)
        ...
        if self.include_audio_to_vision:
            H_v, valid_v = self.vision_enc(vision, vision_lengths, return_seq=True)
            z_v_att, _ = self.attn_a2v(z_a_pool, H_v, valid=valid_v)             # (B,D)
            feats.append(z_v_att)

        # optional include pooled text
...
class MandarinHybridFusionV2(nn.Module):
    ...
    def __init__(
        ...
        self.bicross = BiCrossAttnPoolingAudioMHA(
            d_model=d_model,
            dropout_enc=dropout_enc,
            text_in=text_in,
            audio_in=audio_in,
        ...
        # n_blocks = 3 + (1 if include_audio_to_vision else 0) + (1 if include_pooled_text else 0)
        n_blocks = 2 + (1 if include_audio_to_vision else 0) + (1 if include_pooled_text else 0)
